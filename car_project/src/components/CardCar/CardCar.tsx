    ...
    <>
      <div className='CardCar'>
        ...
        <div className='CarOptions'>
          <div className='CarFirstRow'><div className='CarName'>{Car.name}</div>
            
            <div className='SecondOptions'>{Car.liters}/{Car.Power} л.с/{Car.engineType}<br />{Car.transmissionType}<br />{Car.carType}</div>
            <div className='Price'>{Car.price} ₽</div>
            <MainButton  onClick={() => console.log('1')} label='Купить' arrow={false} btnStyle='SmallButton' link='any' />
          </div>
          <div className='CarSecondRow'><div className='FirstOptions'>{Car.year} <br />{Car.mileage} км</div>
